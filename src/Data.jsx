export const imgData = {
  projects: [
    {
      id: 10,
      project:
        'Servicios miscelaneos de construccion y montaje de estructuras metalicas',
      type: "Mecanico",
      location: "Proyecto construccion de planta de reciclaje de baterias - Chilca",
      country: "Peru",
      client: "MANTTO SAC",
      owner: "ETNA",
      year: 2019,
      images: [
        {
          key: "img58",
          src: "../src/assets/work/pag15/mantto01.png",
          description: "Montaje de Estructura metalica en galpon",
        },
        {
          key: "img59",
          src: "../src/assets/work/pag15/mantto02.png",
          description: "Descarga de estructura metalica",
        },
        {
          key: "img60",
          src: "../src/assets/work/pag15/mantto03.png",
          description: "Vista de estructura metalica montada",
        },
        {
          key: "img61",
          src: "../src/assets/work/pag15/mantto04.png",
          description: "Descarga de material para estructura",
        },
      ],
    },
    {
      id: 9,
      project:
        'Prefabricacion, instalacion de lineas de interconexion de equipos',
      type: "Mecanico",
      location: "Proyecto de cogeneracion central, puerto San Lorenzo - Santa Fe",
      country: "Argentina",
      client: "DVS Construcciones S. A.",
      owner: "Central Puerto",
      year: 2019,
      images: [
        {
          key: "img54",
          src: "../src/assets/work/pag14/cogenedvs01.png",
          description: "Montaje de Tuberia y soporteria",
        },
        {
          key: "img55",
          src: "../src/assets/work/pag14/cogenedvs02.png",
          description: "Montaje de sistema de valvulas en tuberia",
        },
        {
          key: "img56",
          src: "../src/assets/work/pag14/cogenedvs03.png",
          description: "Prefabricacion y acoplamiento de tuberia",
        },
        {
          key: "img57",
          src: "../src/assets/work/pag14/cogenedvs04.png",
          description: "Vista general de la planta",
        },
      ],
    },
    {
      id: 8,
      project:
        'Prefabricacion, instalacion y pruebas hidrostaticas de tuberias de interconexion (30", 24", 16", 12", 4" y 3")',
      type: "Mecanico",
      location: "Central de procesos Tiputini - Bloque 43",
      country: "Ecuador",
      client: "ENERGYPETROL",
      owner: "Petroamazonas",
      year: 2019,
      images: [
        {
          key: "img49",
          src: "../src/assets/work/pag13/energy01.png",
          description: "Tuberia en Pipe Rack",
        },
        {
          key: "img50",
          src: "../src/assets/work/pag13/energy02.png",
          description: "Prefabricacion de tuberia",
        },
        {
          key: "img51",
          src: "../src/assets/work/pag13/energy03.png",
          description: "Prueba Hidrostatica de tuberia",
        },
        {
          key: "img52",
          src: "../src/assets/work/pag13/energy04.png",
          description: "Montaje de tuberia en pipe rack",
        },
        {
          key: "img53",
          src: "../src/assets/work/pag13/energy05.png",
          description: "Soldadura de accesorios de tuberia",
        },
      ],
    },
    {
      id: 7,
      project:
        "Montaje y Construccion de 3 tanques para almacenamiento de derivados de petroleo",
      type: "Mecanico",
      location: "Central de procesos Tiputini - Bloque 43",
      country: "Ecuador",
      client: "Consorcio TDO (Conduto - SEDEMI)",
      owner: "Petroamazonas",
      year: 2019,
      images: [
        {
          key: "img45",
          src: "../src/assets/work/pag12/tdo01.png",
          description: "Montaje deflectores en tanque",
        },
        {
          key: "img46",
          src: "../src/assets/work/pag12/tdo02.png",
          description: "Montaje de seccion de techo del tanque",
        },
        {
          key: "img47",
          src: "../src/assets/work/pag12/tdo03.png",
          description: "Montaje de techo del tanque",
        },
        {
          key: "img48",
          src: "../src/assets/work/pag12/tdo04.png",
          description: "Vista de tanques terminados",
        },
      ],
    },
    {
      id: 6,
      project:
        "Construccion y montaje de sistemas de almacenamiento y abastecimiento de agua fresca",
      type: "Mecanico",
      location: "Marcona - Ica",
      country: "Peru",
      client: "China Metallurgical Group Corporation (MCC)",
      owner: "Shougang Hierro Peru SAA",
      year: 2018,
      images: [
        {
          key: "img41",
          src: "../src/assets/work/pag11/ica01.png",
          description: "Montaje de 2° anillo del tanque",
        },
        {
          key: "img42",
          src: "../src/assets/work/pag11/ica02.png",
          description: "Montaje de cuerpo del tanque",
        },
        {
          key: "img43",
          src: "../src/assets/work/pag11/ica03.png",
          description: "Montaje de techo del tanque",
        },
        {
          key: "img44",
          src: "../src/assets/work/pag11/ica04.png",
          description: "Montaje del tanque terminado",
        },
      ],
    },
    {
      id: 5,
      project:
        "Construccion de tanques de almacenamiento de varias capacidades",
      type: "Mecanico",
      location: "Proyecto central termica Villa Maria - Cordova",
      country: "Argentina",
      client: "DVS Construcciones S. A.",
      owner: "MSU Energy",
      year: 2017,
      images: [
        {
          key: "img37",
          src: "../src/assets/work/pag10/mariadvs01.png",
          description: "Montaje de ultimo anillo con sistema hidraulico",
        },
        {
          key: "img38",
          src: "../src/assets/work/pag10/mariadvs02.png",
          description: "Estructura interna del tanque",
        },
        {
          key: "img39",
          src: "../src/assets/work/pag10/mariadvs03.png",
          description: "Limpieza abrasiva externa",
        },
        {
          key: "img40",
          src: "../src/assets/work/pag10/mariadvs04.png",
          description: "Vista general tanques",
        },
      ],
    },
    {
      id: 4,
      project:
        "Construccion, montaje y pintura de tanques y lineas para sistema de combstible y Sistema Contraincendio",
      type: "Mecanico",
      location: "Proyecto Mirador - Zamora Chinchipe",
      country: "Ecuador",
      client: "SIDERMET",
      owner: "ECSA",
      year: 2016,
      images: [
        {
          key: "img20",
          src: "../src/assets/work/pag06/ecsa01.png",
          description: "Montaje tanques 1425 m3",
        },
        {
          key: "img21",
          src: "../src/assets/work/pag06/ecsa02.png",
          description: "Vista general 3 tanques",
        },
        {
          key: "img22",
          src: "../src/assets/work/pag06/ecsa03.png",
          description: "Vista T-1101A y T1101B",
        },
        {
          key: "img23",
          src: "../src/assets/work/pag06/ecsa04.png",
          description: "T-1101A, T-1101B, T-1401",
        },
        {
          key: "img24",
          src: "../src/assets/work/pag06/ecsa05.png",
          description: "Montaje lineas de tuberia enterrada",
        },
        {
          key: "img25",
          src: "../src/assets/work/pag06/ecsa06.png",
          description: "Pruebas Sistema contraincendio sector bombas",
        },
        {
          key: "img26",
          src: "../src/assets/work/pag06/ecsa07.png",
          description: "Montaje de lineas sistema contraincendio",
        },
        {
          key: "img27",
          src: "../src/assets/work/pag06/ecsa08.png",
          description: "Vista cubeto T-1101A y T-1101B",
        },
        {
          key: "img28",
          src: "../src/assets/work/pag06/ecsa09.png",
          description: "Montaje de fondo de tanque",
        },
        {
          key: "img29",
          src: "../src/assets/work/pag06/ecsa10.png",
          description: "Montaje virolas 3° Anillo",
        },
        {
          key: "img30",
          src: "../src/assets/work/pag06/ecsa11.png",
          description: "Tanque T-4401 Sistema Contra Incendio terminado",
        },
        {
          key: "img31",
          src: "../src/assets/work/pag06/ecsa12.png",
          description: "Alineado y soldadura de lineas de tuberia",
        },
        {
          key: "img32",
          src: "../src/assets/work/pag06/ecsa13.png",
          description: "Tendido de lineas de tuberia enterrada",
        },
        {
          key: "img33",
          src: "../src/assets/work/pag06/ecsa14.png",
          description: "Polikenado de tuberia",
        },
        {
          key: "img34",
          src: "../src/assets/work/pag06/ecsa15.png",
          description: "Polikenado de lineas de tuberia",
        },
        {
          key: "img35",
          src: "../src/assets/work/pag06/ecsa16.png",
          description: "Prefabricacion de tuberia enterrada",
        },
        {
          key: "img36",
          src: "../src/assets/work/pag06/ecsa17.png",
          description: "Izaje y enterrado de lineas de tuberia",
        },
      ],
    },
    {
      id: 3,
      project:
        "Trabajos electricos e instrumentacion especializados para puesta en marcha de pozos",
      type: "Electrico",
      location: "Bloque 7 - Orellana",
      country: "Ecuador",
      client: "PETROAMAZONAS",
      owner: "PETROAMAZONAS",
      year: 2015,
      images: [
        {
          key: "img12",
          src: "../src/assets/work/pag04/elec01.png",
          description: "Tablero electrico",
        },
        {
          key: "img13",
          src: "../src/assets/work/pag04/elec02.png",
          description: "Tablero electrico",
        },
        {
          key: "img14",
          src: "../src/assets/work/pag04/elec03.png",
          description: "Conexionado electrico",
        },
        {
          key: "img15",
          src: "../src/assets/work/pag04/elec04.png",
          description: "Transformadores electicos",
        },
        {
          key: "img16",
          src: "../src/assets/work/pag04/elec05.png",
          description: "Paneles electricos",
        },
        {
          key: "img17",
          src: "../src/assets/work/pag04/elec06.png",
          description: "Generacion electrica",
        },
        {
          key: "img18",
          src: "../src/assets/work/pag04/elec07.png",
          description: "Conexionado electrico",
        },
        {
          key: "img19",
          src: "../src/assets/work/pag04/elec08.png",
          description: "Paneles electricos",
        },
      ],
    },
    {
      id: 2,
      project:
        "Construccion, Montaje y pintura de 7 tanques de almacenamiento (Varias capacidades)",
      type: "Mecanico",
      location: "Proyecto reserva de generacion fria ILO - MOQUEGUA",
      country: "Peru",
      client: "Santos CMI",
      owner: "Santos CMI",
      year: 2012,
      images: [
        {
          key: "img07",
          src: "../src/assets/work/pag03/Moquegua01.png",
          description: "Montaje de tanques",
        },
        {
          key: "img08",
          src: "../src/assets/work/pag03/Moquegua02.png",
          description: "Montaje de tanques",
        },
        {
          key: "img09",
          src: "../src/assets/work/pag03/Moquegua03.png",
          description: "Montaje de tanques",
        },
        {
          key: "img10",
          src: "../src/assets/work/pag03/Moquegua04.png",
          description: "Montaje de tanques",
        },
        {
          key: "img11",
          src: "../src/assets/work/pag03/Moquegua05.png",
          description: "Montaje de tanques",
        },
      ],
    },
    {
      id: 1,
      project: "Reubicacion de lineas y Sistema de bombas del area de soda",
      type: "Mecanico",
      location: "Terminal Vopak",
      country: "Ecuador",
      client: "VOPAK del Ecuador S. A.",
      owner: "VOPAK del Ecuador S. A.",
      year: 2007,
      images: [
        {
          key: "img04",
          src: "../src/assets/work/pag02/Vopak01.png",
          description: "Reubicacion de lineas",
        },
        {
          key: "img05",
          src: "../src/assets/work/pag02/Vopak02.png",
          description: "Reubicacion de lineas",
        },
        {
          key: "img06",
          src: "../src/assets/work/pag02/Pascuales03.png",
          description: "Reubicacion de bombas",
        },
      ],
    },
    {
      id: 0,
      project:
        "Construccion e instalacion de interconexion de lineas de diesel y gasolina",
      type: "Mecanico",
      location: "Terminal Pascuales",
      country: "Ecuador",
      client: "MOVINSER S. A.",
      owner: "PETROECUADOR",
      year: 2007,
      images: [
        {
          key: "img01",
          src: "../src/assets/work/pag02/Pascuales01.png",
          description:
            "Construccion e instalacion de lineas de diesel y gasolina",
        },
        {
          key: "img02",
          src: "../src/assets/work/pag02/Pascuales02.png",
          description:
            "Construccion e instalacion de lineas de diesel y gasolina",
        },
        {
          key: "img03",
          src: "../src/assets/work/pag02/Pascuales03.png",
          description:
            "Construccion e instalacion de lineas de diesel y gasolina",
        },
      ],
    },
  ],
};
